...
HRESULT CreateDeviceD3D(HWND hWnd)
{
	...
	if (D3D10CreateDeviceAndSwapChain(NULL,
		...
		, NULL, createDeviceFlags, D3D10_SDK_VERSION, &sd, &g_pSwapChain, &g_pd3dDevice) != S_OK) {
		printf("WARN: DX10 HW FAIL");
		return E_FAIL;
		if (D3D10CreateDeviceAndSwapChain(NULL,
			D3D10_DRIVER_TYPE_WARP, NULL, createDeviceFlags, D3D10_SDK_VERSION, &sd, &g_pSwapChain, &g_pd3dDevice) != S_OK)
		{
			printf("WARN: DX10 SW FAIL");
			return E_FAIL;
		}
	}
...
API bool setupAll_dx10() {
	...
	RegisterClassEx(&wc);
	hwnd = CreateWindow(_T("ImGui Example"), _T("ImGui DirectX10 Example"), WS_OVERLAPPEDWINDOW, 100, 100, 1280, 800, NULL, NULL, wc.hInstance, NULL);
